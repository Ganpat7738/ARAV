...
def main(text):
		...
		if isContain(text, ['charge','charging','battery', 'system info']):
		...
		if isContain(text, ['meaning', 'dictionary', 'definition', 'define','means','mean']):
		...
		if isContain(text, ['smart board','air canvas','canvas','hair canvas']):
			aircanvas()
			return
		...
		if isContain(text, ['timer', 'countdown','set']):
		...
		if isContain(text, ['factorial','log ','value of','math',' + ',' - ','/',' x ','multiply','divide' ,'divided by','binary','hexadecimal','octal','shift','sin ','cos ','tan ']):
			try:
		...
		if isContain(text, ['wiki', 'who is','wikipedia']):
		...
		if isContain(text, ['game','play game']):
		...
		if isContain(text,['deactivate','quit','exit program']):
			exit()

		if isContain(text,['aircanvas','air canvas','smart board']):
			import air
			return air.aircanvas()

		result = normalChat.reply(text)
		if result != "None": speak(result, True, True)
		else:
			speak(" please say that again ! ", True, True)
...
def showSingleImage(type, data=None):
	...
	if type=="weather":
		...
		img3 = ImageTk.PhotoImage(Image.open('D:\\PersonalAssistantChatbot-main\\extrafiles\\dice\\'+type+ '.jpg').resize((200,200), Image.ANTIALIAS))
		Label(chat_frame, image=img3, bg='#EAEAEA').pack(anchor='w')
	# 'extrafiles/images/dice/'+type+'.jpg'
